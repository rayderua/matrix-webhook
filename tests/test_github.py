...
class GithubFormatterTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_github_push(self):
        ...
        GH = "https://github.com"
        expected = f'<p><a href="{GH}/zerodotfive">@zerodotfive</a> pushed on refs/heads/devel: '
        expected += f'<a href="{GH}/zerodotfive/matrix-webhook/compare/ac7d1d964700...'
        expected += f'4bcdb25c8093">{before} → {after}</a>:</p>\n<ul>\n<li>'
        expected += f'<a href="{GH}/zerodotfive/matrix-webhook/commit/{after}">'
